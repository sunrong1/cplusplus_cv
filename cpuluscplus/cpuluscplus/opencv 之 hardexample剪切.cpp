...
int main()
{
	...
	string ImgName;
	ifstream fin_detector("detector64.txt");//打开自己训练的SVM检测器文件
	ifstream fin_imgList("posbig.txt");//打开原始负样本图片文件列表
	//ifstream fin_imgList("subset.txt");
		// ifstream fin_imgList("neghard.txt");
		// while(getline(fin_imgList,ImgName))
		// {
			// string fullName = "F:/OpenCVproject/data/INRIA/INRIAPerson/Test/neghard/" + ImgName;//加上路径名
	...
	//从文件中读入自己训练的SVM参数
	float temp;
	vector<float> myDetector;//3781维的检测器参数,(必须为3781维？）
	...
	while(getline(fin_imgList,ImgName))
	{
		...
		//string fullName = "F:/OpenCVproject/data/INRIA/INRIAPerson/Train/neg/" + ImgName;//加上路径名
		string fullName = "F:/OpenCVproject/data/INRIA/INRIAPerson/Test/pos/" + ImgName;//加上路径名
		src = imread(fullName);//读取图片
