...
//精确检测人体目标，解决自遮挡和互遮挡的问题
#include <iostream>
#include <fstream>   //读写已命名文件的类型,包括ifstream ofstream
#include <string>
#include <sstream>	//读写内存中string对象 ostringstream
...
//类的实现部分  作用域运算符::是用来标识某个成员函数是属于哪个类的
...
void RunFiles()
{
   ...
   /* const char* listname = "video2/files.txt";
    ...
    while(in.good())
    {
        ...
        buf.str(""); buf<<"F:/OpenCVproject/data/TUD/tud-campus-sequence/"<<filename;
	...
	outf<<"Processed "<<numfile<<" images in "<<totaltime<<" milliseconds."<<std::endl;
    in.close();
	out.close();
}
