...
void App::run()
{
	...
	ifstream in("posbig.txt");
	//ifstream in("tudcam.txt");
	string fileName;
    ...
    while (running)
    {
        ...
        {
			//in>>fileName;
			////frame=imread(("F:/OpenCVproject/data/TUD/tud-campus-sequence/"+fileName));
			//frame=imread(("F:/OpenCVproject/data/INRIA/INRIAPerson/Test/pos/"+fileName));
        ...
        while (running && !frame.empty())
        {
            ...
            if (abs(scale-1.0)>0.001)
            {
              ...
              /*  if (use_gpu)
                {
					ocl::resize(gpu_img_resize, gpu_img, sz);
                }
				else */
					{
						resize(img_aux, img, sz);
    ...
    //        if (vdo_source!="" || camera_id!=-1) vc >> frame;
		...
		totaltime += EndOfDuration();
		   std::cout<<"Processed "<<" images in "<<totaltime<<" milliseconds."<<std::endl;
    }
}
