...
#define PosSamNO 2400    //正样本个数 2400 2416
#define NegSamNO 12000    //负样本个数12000 12180
...
#define HardExampleNO 4000  //4435
...
int main()
{
	...
	if(TRAIN)
	{
		...
		ifstream finPos("pos96.txt");//正样本图片的文件名列表
		//ifstream finPos("PersonFromVOC2012List.txt");//正样本图片的文件名列表
		ifstream finNeg("neg64.txt");//负样本图片的文件名列表
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = "../../../data/INRIA/INRIAPerson/96X160H96/Train/pos/" + ImgName;//加上正样本的路径名
		...
		{
			cout<<"处理："<<ImgName<<endl;
			ImgName = "../../../data/INRIA/INRIAPerson/neg/" + ImgName;//加上负样本的路径名
			Mat src = imread(ImgName);//读取图片
		...
		if(HardExampleNO > 0)
		{
			ifstream finHardExample("neghard.txt");//HardExample负样本的文件名列表
			cout<<"处理："<<"hardexample............................................."<<endl;
			//依次读取HardExample负样本图片，生成HOG描述子
			for(int num=0; num<HardExampleNO && getline(finHardExample,ImgName); num++)
			{
				cout<<"处理："<<ImgName<<endl;
				ImgName = "../../../data/INRIA/INRIAPerson/Test/neghard/" + ImgName;//加上HardExample负样本的路径名
				Mat src = imread(ImgName);//读取图片
				//resize(src,img,Size(64,128));
		...
		cout<<"训练完成"<<endl;
		svm.save("SVM_HOG_test1.xml");//将训练好的SVM模型保存为xml文件

	}
	else //若TRAIN为false，从XML文件读取训练好的分类器
	{
		svm.load("SVM_HOG_test1.xml");//从XML文件读取训练好的SVM模型 SVM_HOG_2400PosINRIA_12000Neg_HardExample(误报少了漏检多了).xml
	}
	...
	***************************************************************************************************/
	...
	for(int i=0; i<supportVectorNum; i++)
	{
		alphaMat.at<float>(0,i) = pAlphaData[i]; //二维单通道元素可以用Mat::at(i, j)访问，i是行序号，j是列序号。
	}
	...
	ifstream finPos("posbig.txt");//正样本图片的文件名列表
	string ImgName;
	for(int num=0; num< 100; num++)
	{
		getline(finPos,ImgName);
		cout<<"处理："<<ImgName<<endl;
		ImgName = "../../../data/INRIA/INRIAPerson/Test/pos/" + ImgName;//加上负样本的路径名
